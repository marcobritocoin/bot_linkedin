...
async function main() {
    ...
    const user = "marcobritoduran@gmail.com";
    const pass = "Marcopolo85";
    const timeWait = 200;
                ...
                function reviewSetWin(textDefault, numMovil){
                    ...
                    var selE, inputElement, tipoInput, titleInput, tagID;
                        ...
                        for(var i=0; i< totalElem; i++){	
                                ...
                                if(tipoInput == "SELECT"){
                                    if(titleInput.toLowerCase().indexOf('email') == -1 && titleInput.toLowerCase().indexOf('phone country') == -1){
                                        console.log('Input tipo Select');
                                    ...
                                    if(inputElement.getAttribute('type') == 'radio'){
                                            ...
                                            selectorium.childNodes.forEach((e)=>{
                                                if(e.innerText?.trim().toLowerCase().indexOf('yes') >= 0){
                                                    e.children[0].click();
                                                    finder=true;
                                                }
                                            });
...
async function cerrarPopupFinal(page){
    await page.evaluate(()=>{
        try { 
            //document.querySelector('[class="artdeco-button__icon"]').click();
